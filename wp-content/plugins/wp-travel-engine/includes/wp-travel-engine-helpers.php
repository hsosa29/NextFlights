 ...
 * @return Array
 */
function wp_travel_engine_get_trips_array() {
	$args = array(
		'post_type'   => 'trip',
		'numberposts' => -1,
	);
...
function wp_travel_engine_get_prev_price( $trip_id ) {
	...
	if( $trip_settings ) {
		$prev_price = isset( $trip_settings['trip_prev_price'] ) && ! empty( $trip_settings['trip_prev_price'] ) ? $trip_settings['trip_prev_price'] : 0;
	}
...
function wp_travel_engine_get_sale_price( $trip_id ) {
	...
	if( $trip_settings ) {
		$sale_price = isset( $trip_settings['trip_price'] ) && ! empty( $trip_settings['trip_price'] ) ? $trip_settings['trip_price'] : 0;
	}
 ...
 * @return void
 */
function wp_travel_engine_get_currency_code() {
	...
	return apply_filters( 'wp_travel_engine_currency_code', $code );
