...
class Wp_Travel_Engine_Admin
{
	...
	public function enqueue_styles()
	{
		...
		if ($screen->post_type == 'trip' || $screen->post_type == 'enquiry' || $screen->post_type == 'booking' || $screen->post_type == 'customer' || isset($_GET['page']) && $_GET['page'] == 'class-wp-travel-engine-admin.php' || $screen->id == 'trip_page_class-wp-travel-engine-admin' || $screen->post_type == 'downloadfile') {
			wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/wp-travel-engine-admin.css', array(), $this->version, 'all');
		}
		if ($screen->post_type == 'booking' || $screen->post_type == 'customer' || isset($_GET['page']) && $_GET['page'] == 'reviews') {
	...
	public function enqueue_scripts()
	{
		...
		if ($screen->post_type == 'trip' || $screen->post_type == 'booking' || isset($_GET['page']) && $_GET['page'] == 'class-wp-travel-engine-admin.php' || $screen->id == 'trip_page_class-wp-travel-engine-admin') {
			...
			wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/dist/wp-travel-engine-admin-'. WP_TRAVEL_ENGINE_VERSION .'.js', array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-sortable'), $this->version, true);
			...
			wp_enqueue_script($this->plugin_name . 'media-logo-upload', plugin_dir_url(__FILE__) . 'js/dist/media-upload-'. WP_TRAVEL_ENGINE_VERSION .'.js', array('jquery'), $this->version, true);
		}
		...
		if ($screen->post_type == 'trip' || $screen->post_type == 'booking' || $screen->post_type == 'customer') {
			...
			wp_enqueue_script($this->plugin_name . 'custom', plugin_dir_url(__FILE__) . 'js/dist/custom-'. WP_TRAVEL_ENGINE_VERSION .'.js', array('jquery'), $this->version, true);
		}
		if ($screen->post_type == 'trip') {
			
			wp_enqueue_script($this->plugin_name . 'gallery-metabox', plugin_dir_url(__FILE__) . 'js/dist/gallery-metabox-'. WP_TRAVEL_ENGINE_VERSION .'.js', array('jquery'), $this->version, true);
		...
		wp_enqueue_script('select2', plugin_dir_url(__FILE__) . 'js/dist/select2-'. WP_TRAVEL_ENGINE_VERSION .'.js', array('jquery'), '5.6.3', true);
	}
	...
	public function messages_page()
	{
		...
		add_submenu_page('edit.php?post_type=trip', 'WP Travel Engine Admin Messages', $menu_title, 'manage_options', 'wte-messages', array($this, 'display_messages_page'));
	}
	...
	function wp_add_trip_info()
	{
		...
		$response = '<li class="trip_facts">';
		$icon = isset($wp_travel_engine_option_settings['trip_facts']['field_icon'][$key]) ? esc_attr($wp_travel_engine_option_settings['trip_facts']['field_icon'][$key]) : '';
	...
	*/
	...
	function wpte_trip_pay_add_meta_boxes()
	{
		...
		foreach ($screens as $screen) {
			...
			add_meta_box(
				'wp_travel_engine_payment_details',
				__('Payment Details', 'wp-travel-engine'),
				array( $this, 'wp_travel_engine_payment_metabox_callback' ),
				$screen,
				'side',
				'high'
			);
	 ...
	 * @return void
	 */
	public function wp_travel_engine_payment_metabox_callback() {
		include WP_TRAVEL_ENGINE_BASE_PATH . '/includes/backend/booking/payment-status.php';
	}
	...
	public function in_plugin_update_message($plugin_data, $response)
	{
		...
		if ($compatibility_check->requires_backward_processs()) {
