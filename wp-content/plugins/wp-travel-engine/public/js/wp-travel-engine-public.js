  jQuery(document).ready(function($){
    ...
    if( $('input[name=wp_travel_engine_payment_mode]').length > 0 ){
      ...
      $(document).on('change', '[name=wp_travel_engine_payment_mode]', function(){
        ...
        amount = wteGetFormatedPrice( amount );

        'partial' == payment_mode ? partial_payment_table.show() : partial_payment_table.hide();

        $('.wpte-bf-book-summary .wpte-bf-total-price .wpte-bf-price-amt').text(amount);
